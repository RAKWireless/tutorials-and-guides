...
static uint16_t C1FIFOCON_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t C1FIFOSTA_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t C1FIFOUA_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t C1FLTCON_REGISTER(const uint16_t inFilterIndex)
...
static uint16_t C1FLTOBJ_REGISTER(const uint16_t inFilterIndex)
...
static uint16_t C1MASK_REGISTER(const uint16_t inFilterIndex)
                         ...
                         const ACAN2517Filters &inFilters)
{
  ...
  if (errorCode == 0)
  {
    ...
    // writeRegister32(0x400, 0x55);
    // const uint32_t xxx = readRegister32(0x400);
    // Serial.print("xxx=");
    // Serial.println(xxx, HEX);
...
//
#else
  noInterrupts(); // For ensuring mutual exclusion access
#endif
  const bool hasReceivedMessage = mDriverReceiveBuffer.count() > 0;
#if defined ARDUINO_ARCH_ESP32
  taskENABLE_INTERRUPTS();
...
//   { // Receive FIFO is not full, enable "FIFO  not empty" interrupt
...
bool ACAN2517::recoverFromRestrictedOperationMode(void)
{
  ...
  if (currentOperationMode() == ACAN2517::RestrictedOperation)
...
uint32_t ACAN2517::diagInfos(const int inIndex)
