...
uint32_t voltage, rpm, oilpressure;
...
void loop()
{
  ...
  voltage = returnCANdata1();
  rpm = returnCANdata2();
  oilpressure = returnCANdata3();
  Serial.printf("voltage: %d, RPM: %d, Oilpressure: %d \n", voltage, rpm, oilpressure);
...
void send_lora_frame(void)
{
  ...
  m_lora_app_data.buffer[i++] = (uint8_t)(voltage >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)(voltage);
  m_lora_app_data.buffer[i++] = (uint8_t)(rpm >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)(rpm);
  m_lora_app_data.buffer[i++] = (uint8_t)(oilpressure >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)(oilpressure);
