...
void fillBuffer()
{
  ...
  m_lora_app_data.port = gAppPort;
  uint16_t heartRate = returnValue();// Read the average heartrate

  data = String(heartRate) + " BPM";
  Serial.println(data);

  //Create the payload
  m_lora_app_data.buffer[i++] = 0x01;
  m_lora_app_data.buffer[i++] = (uint8_t)(heartRate >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)heartRate;
