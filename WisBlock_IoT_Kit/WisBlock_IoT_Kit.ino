...
//Replace your OTAA keys here!!!! KEYS ARE MSB !!!!
uint8_t nodeDeviceEUI[8] = { 0x70, 0xB3, 0xD5, 0x7E, 0xD0, 0x05, 0xB1, 0x58 };
uint8_t nodeAppEUI[8] = { 0x31, 0x96, 0x85, 0x46, 0x51, 0x65, 0x13, 0x21 };
uint8_t nodeAppKey[16] = { 0xDB, 0x5A, 0x08, 0x0D, 0x28, 0xD6, 0x6C, 0x8A, 0xD2, 0x7E, 0x00, 0x74, 0xB6, 0x71, 0x57, 0xF9 };
...
void setup() {
  ...
  /* Light sensor and temperature sensor init */
  configSensorLIGHT();
  configSensorTEMPandHUMD();
...
void fillBuffer() {
  ...
  m_lora_app_data.port = gAppPort;
  uint16_t lightlux = returnlux();
  uint16_t lightwhite = returnwhite();
  uint16_t lightRawALS = returnRAWALS();
  data = String(lightlux) + " lux" + String(lightwhite) + "white" + String(lightRawALS) + "RawALS";
  Serial.println(data);

  //Create the payload
  m_lora_app_data.buffer[i++] = 0x01;
  m_lora_app_data.buffer[i++] = (uint8_t)(lightlux >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)lightlux;
  m_lora_app_data.buffer[i++] = (uint8_t)(lightwhite >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)lightwhite;
  m_lora_app_data.buffer[i++] = (uint8_t)(lightRawALS >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)lightRawALS;
